...
{
  $('#tombol').empty();
  $('#tombol').append('<a type="button" id="hidelegenda" onclick="hideLegenda()" class="btn btn-default " data-toggle="tooltip" title="Hide the Legend" style="margin-right: 7px;"><i class="fa fa-eye-slash"></i></a> ');
        ...
        { 
         for (var i in rows) 
          { 
            ...
            content: "<center><span style=color:black><b>Information</b><table><tr><td><i class='fa fa-home'></i>Name</td><td>:</td><td> "+namaa+"</td></tr><br><tr><td><i class='fa fa-map-marker'></i>Address</td><td>:</td><td> "+address+"</td></tr><br><tr><td><i class='fa fa-phone'></i>Phone</td><td>:</td><td> "+cp+"</td></tr></table></span>",   
        ...
        { 
         for (var i in rows) 
          { 
            ...
            infowindow = new google.maps.InfoWindow({
            position: centerBaru,
            content: "<center><span style=color:black><b>Information</b><table><tr><td><i class='fa fa-home'></i>Mosque's Name</td><td>:</td><td> "+name+"</td></tr><br><tr><td><i class='fa fa-map-marker'></i>Address</td><td>:</td><td> "+address+"</td></tr><br><tr><td><i class='fa fa-building'></i>Capacity</td><td>:</td><td> "+capacity+"</td></tr></table></span>",   
            pixelOffset: new google.maps.Size(0, -33)
            });
          infoposisi.push(infowindow); 
          hapusInfo();
      ...
      if(rows==null)
        ...
        for (var i in rows) 
            {   
              ...
              $('#hasilcari').append("<tr><td>"+nama_tempat_kuliner+"</td><td><a role='button' class='btn btn-success fa fa-info' title='info' onclick='detculi(\""+id+"\");detailinfokul(\""+id+"\");'></a></td><td><a role='button' class='btn btn-danger fa fa-taxi' onclick='kulAngkot(\""+id+"\")'></a></td></tr>");
            }
              ...
              for (var i in rows.features) 
                { 
                  ...
                  var infowindow = new google.maps.InfoWindow({
                    position: centerBaru,
                    content: "<bold>INFORMATION</bold><br>Route Code: "+id+"<br>Destination: "+destination+"<br>Angkot's Track: "+track+"",
                  });
                  infowindow.open(map);                    
...
{
  hapusawal();
  var fas=selectthifa.value;
  var arrayLay=[];
  for(i=0;i<$("input[name=thifa]:checked").length;i++){
    arrayLay.push($("input[name=thifa]:checked")[i].value);
  }
  console.log('zz');
  if (arrayLay==''){
    alert('Choose Culinary');
    ...
    $('#hasilcari').append("<thead><th>Name</th><th colspan='3'>Action</th></thead>");
    console.log(server+'thifa.php?lay='+arrayLay);
    $.ajax({ url: server+'thifa.php?lay='+arrayLay, data: "", dataType: 'json', success: function(rows){
      console.log("hai");
      if(rows==null)
            {
              alert('Data not found');
    ...
    { 
      ...
      else
      {
        ...
        { 
          ...
          clickMarker(centerBaru, id);
          map.setCenter(centerBaru);
           $('#hasilcari').append("<tr><td>"+name+"</td><td><a role='button' class='btn btn-success fa fa-info' title='info' onclick='detculi(\""+id+"\");detailinfokul(\""+id+"\");'></a></td><td><a role='button' class='btn btn-danger fa fa-taxi' onclick='kulAngkot(\""+id+"\")'></a></td></tr>");
        }
      } 
      //$('#hasilpencarian').append("<h5 class='box-title' id='hasilpencarian'>Result :</h5>"+rows.length);
    }
        ...
        { 
         ...
         for (var i in rows) 
          { 
            ...
            infowindow = new google.maps.InfoWindow({
            position: centerBaru,
            content: "<center><span style=color:black><b>Information</b><table><tr><td><i class='fa fa-home'></i>Name</td><td>:</td><td> "+namaa+"</td></tr><br><tr><td><i class='fa fa-map-marker'></i>Address</td><td>:</td><td> "+address+"</td></tr><br><tr><td><i class='fa fa-phone'></i>Phone</td><td>:</td><td> "+cp+"</td></tr><br><tr><td><i class='fa fa-clock-o'></i>Open</td><td>:</td><td> "+open+"</td></tr><br><tr><td><i class='fa fa-clock-o'></i>Close</td><td>:</td><td> "+close+"</td></tr><br><tr><td><i class='fa fa-building'></i>Capacity</td><td>:</td><td> "+capacity+"</td></tr></table></span><br><a class='btn btn-default fa fa-road' role=button' onclick='callRoute(centerLokasi,centerBaru);rutetampil()' title='Route' aria-controls='Route' ></a>&nbsp<a class='btn btn-default fa fa-info' role=button' onclick='gallery(\""+id+"\")'title='Info' aria-controls='Info'></a>&nbsp<a class='btn btn-default fa fa-compass' role=button' onclick='tampil_sekitar(\""+latitude+"\",\""+longitude+"\",\""+namaa+"\")'title='Nearby' aria-controls='Nearby'></a> ",   
            pixelOffset: new google.maps.Size(0, -33)
            });
          infoposisi.push(infowindow); 
          hapusInfo();
...
function find_kul() //kuliner
{
  ...
  if(kul_nama.value=='')
  {
    alert("Fill in the Search Field First!");
  }
  else
  {
    $('#hasilcari').append("<thead><th>Name</th><th colspan='3'>Action</th></thead>");
        ...
        { 
         for (var i in rows) 
          { 
            ...
            infowindow = new google.maps.InfoWindow({
            position: centerBaru,
            content: "<center><span style=color:black><b>Information</b><br><table><tr><td><i class='fa fa-home'></i>Hotels Name</td><td>:</td><td> "+name+"</td></tr><br><tr><td><i class='fa fa-map-marker'></i>Alamat</td><td>:</td><td> "+address+"</td></tr><br><tr><td><i class='fa fa-phone'></i>Fasilitas</td><td>:</td><td> "+cp+"</td></tr></table></span>",   
            pixelOffset: new google.maps.Size(0, -33)
            });
          infoposisi.push(infowindow); 
          hapusInfo();
...
function callRoute(start, end)
{
  ...
  {
    alert('Click the Button of Current Position Beforehand');
  }
  else
  {
  	$('#hasilrute').show();
